...
from config import (
    TOTAL_DAILY_USERS, MAX_PARALLEL_USERS, JOURNEY_RANGES,
...
def run_single_bot(session_id, journey_name):
    """The isolated thread worker executing a full user lifecycle."""
    cdp_url = os.environ.get("CHROME_CDP", "http://127.0.0.1:9222")
 
    with sync_playwright() as p:
        try:
            # 1. ROLL THE DICE: Mobile vs Desktop
            target_mweb_pct = random.randint(MWEB_PERCENTAGE_RANGE[0], MWEB_PERCENTAGE_RANGE[1])
            is_mobile = random.randint(1, 100) <= target_mweb_pct
           
            # Connect to the background Chrome instance (or GitHub Actions service)
            browser = p.chromium.connect_over_cdp(cdp_url)
           
            # 2. APPLY THE DISGUISE
            # Fetch your baseline options first, then overwrite with device specifics
            context_kwargs = build_context_options()
           
            if is_mobile:
                print(f"[Session #{session_id}] 📱 Connecting via CDP -> Route: {journey_name} (Mobile/iPhone 13)")
                context_kwargs.update(p.devices['iPhone 13'])
                context = browser.new_context(**context_kwargs)
            else:
                print(f"[Session #{session_id}] 💻 Connecting via CDP -> Route: {journey_name} (Desktop/1080p)")
            ...
            # THE HACK: Attach the mobile flag directly to the page object
            # so navigation.py can read it without altering journey files!
            ...
            if journey_module:
                # The journey receives 'page', which now carries page.is_mobile
