...
# =====================================================================
# --- CORE HUMAN SIMULATION HELPERS (HEAVILY DELAYED) ---
# =====================================================================
...
def human_click_element(element):
    ...
    human_delay(2.0, 4.0)  # EXTENDED: Human reading/thinking time
...
def click_random_category(page):
    ...
    try:
        # 1. MOBILE CHECK: If it's a mobile bot, open the hamburger menu first
        if getattr(page, 'is_mobile', False):
            print("  -> [Nav] Mobile detected: Opening hamburger menu...")
            page.pause()
           
            # 🚨 IMPORTANT: Update this selector to match your React hamburger menu button!
            mobile_menu_btn = page.locator("button.hamburger-icon, button[aria-label='Menu']").first
           
            if mobile_menu_btn.count() > 0:
                human_click_element(mobile_menu_btn)
                human_delay(1.0, 2.0) # Wait for the menu animation to slide open
            else:
                print("  -> [Nav] Warning: Hamburger menu button not found.")
 
        # 2. Proceed to click the category links
        # EXTENDED: Wait up to 15 seconds for category links to exist
...
def view_random_product(page):
    ...
    try:
        # EXTENDED: Wait up to 45 seconds to bypass backend rate-limit API delays
...
def add_to_cart(page):
    ...
    try:
        # 1. Force the bot to freeze until React renders the button
        page.wait_for_selector(selectors.ADD_TO_CART_TEXT, state="visible", timeout=15000)
       
        # 2. Once visible, grab it and click
...
def fill_checkout_form(page):
    ...
    try:
        # EXTENDED: Wait up to 45 seconds for your actual first name field to render
